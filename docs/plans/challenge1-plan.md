...
3. **System presents one row per address, with a per-source breakdown, not one opaque score.** This is the direct answer to what agent.md's own persona "Tom" says he wants: *"compare public sources and see how the tool reached its conclusion"* (agent.md, "Officer context"). Sketch, extending the worked-example table from the Examples & Technical Guidance page:
   ...
   | Example St 20 | Hair Salon Example (not in register at this address) | — | Listed, open | Not tagged | Recent reviews cite this address | Called, confirmed open | Medium | For review: establishment missing or wrong address |
...
1. **Data ingestion**
   ...
   - Link an establishment to its parent enterprise via `Ondernemingsnr_maatsch_zetel`. Be aware that most parents are not present in the sample (only 28 of 543 establishments have a parent among the 1,000 rows) — the tool should degrade gracefully (show "parent enterprise not in sample") rather than fail.
